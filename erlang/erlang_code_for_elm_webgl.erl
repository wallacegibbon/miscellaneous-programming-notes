...
-type mesh(Attributes) :: {mesh1, render_info(), [Attributes]} |
...
-spec webgl_entity(shader(Attributes, Uniforms, Varyings),
                   shader(none, Uniforms, Varyings),
                   mesh(Attributes),
                   Uniforms) -> entity().
...
simple_test() ->
    webgl_entity(my_vertex_shader(),
                 my_fragment_shader(),
                 {mesh1, default_render_info(),
                  [#{position => {0.3, 0.5, 0}, color => {0.5, 0.5, 0.5}}]},
                 #{offset_x => 0.2}).
